...
describe.skip('Tweet', () => {
  it('should fetch remote data', async () => {
    let spy = await getFetchSpy(Tweets.text)
    let { element } = await createTweet(Tweets.text)
  ...
  it('should generate the tweet header', async () => {
    ...
    let { query } = await createTweet(Tweets.text)
  ...
  it('should show images', async () => {
    ...
    let { query } = await createTweet(Tweets.image)
  ...
  it('a[target=_blank] should have a [rel=noopener] attribute', async () => {
    ...
    let { element } = await createTweet(Tweets.link)
    let links = [...element.shadowRoot!.querySelectorAll('a[target=_blank')]
  ...
  it('tweet link should contain the username and the tweet id', async () => {
    await getFetchSpy(Tweets.text)
    let { element } = await createTweet(Tweets.text)
    let links = [...element.shadowRoot!.querySelectorAll('.tweet__link')]
  ...
  describe('Tweet body', () => {
    ...
    it('should replace @user', async () => {
      ...
      let { query, element } = await createTweet(Tweets.answeredTweet)
      let a = query('article a[href="https://twitter.com/VictorVEnciso"]')
    ...
    it('should show thread and inherit include-thread attribute on being set', async () => {
      ...
      let parent = element.shadowRoot!.querySelector('embetty-tweet')
